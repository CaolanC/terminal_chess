...
def pieceMove(move):
    ...
    if ord(move[0]) in range(ord('a'), ord('i')):
        ...
        if not white() and move.split('-')[0][1] == '7': #black
            for i in range(6,4, -1):
                pPlaces.append(move[0] + str(i))
            if move.split('-')[1] in pPlaces:
                if move.split('-')[1][-1] == '5':
                    benPS[move.split('-')[1][0] + str(int(move.split('-')[1][1]) + 1)] = move.split('-')[1]
                return True
            else:
                piece = False
        elif not white() and move.split('-')[0][1] < '7':
            if move.split('-')[1] == move.split('-')[0][0] + str(int(move.split('-')[0][1]) + 1) and board[move.split('-')[0][0] + str(int(move.split('-')[0][1]) + 1)] == '.':
                piece = True
            else:
                piece = False
        if not white() and move.split('-')[1] == chr(ord(move.split('-')[0][0]) + 1) + str(int(move.split('-')[0][1]) + 1):
            if board[move.split('-')[1]] != '.':
                piece = True
            else:
                piece = False
