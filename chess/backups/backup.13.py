    ...
    for piecelocation in legalmoves.legalInfo(board, white()):
        for i in piecelocation:
            for j in piecelocation[i]:
                if not isPawn(i):
                    cmove = (i + '-' + j)
                    print('cP', corPiece(cmove),'pM', pieceMove(cmove), 'cm', cmove)
                    if pieceMove(cmove):
                        if kingMove(cmove) and white():
                            wkposh = wking
                        elif kingMove(cmove) and not white():
                            bkposh = bking
                        move(cmove)
                        if white() and not check(wking):
                            legalM.append(cmove)
